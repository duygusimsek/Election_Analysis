...
with open(file_to_load) as election_data:
    ...
    for row in reader:
        ...
        #DS county_names changed to county
...
with open(file_to_save, "w") as txt_file:
    ...
    for county in county_votes:
        ...
        county_vote_perc=float(county_vote_count/total_votes)*100
        # 6d: Print the county results to the terminal.
        county_results=(f"{county}: {county_vote_perc:.1f}% ({county_vote_count:,})\n")
        print(county_results)
         # 6e: Save the county votes to a text file.
        txt_file.write(county_results)
         # 6f: Write an if statement to determine the winning county and get its vote count.
        if (county_vote_count > county_largest_turnout_votes):
            county_largest_turnout_votes = county_vote_count
